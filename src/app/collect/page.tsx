...
export default function CollectPage() {
  ...
  const translateAddress = () => {
    ...
    // Translate detail: replace common Chinese address words with English
    detailEn = rest
    ...
    // Remove remaining Chinese chars and clean up
  ...
  return (
    <div className="min-h-screen bg-white max-w-[100vw] overflow-x-hidden">
      ...
      <div className="px-4 pt-6 pb-4">
        <h1 className="text-xl font-bold text-gray-900">信息收集表</h1>
      </div>
      ...
      <div className="px-4 pb-8">
        <form onSubmit={handleSubmit} className="touch-pan-y">
            ...
            {tabs.map(t=>(
              <button key={t.id} type="button" onClick={()=>{setTab(t.id);setErrors([])}}
                className={`pb-2.5 text-sm whitespace-nowrap border-b-2 transition-all ${
                  tab===t.id ? 'text-gray-900 border-primary-500 font-medium' : 'text-gray-400 border-transparent'
                }`}>{t.label}</button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            {i < tabs.length-1 ? (
              <button type="button" onClick={next} className="px-6 py-2.5 bg-primary-500 text-white rounded-lg text-sm font-medium">下一步 →</button>
            ) : (
              <button type="submit" disabled={submitting} className="px-8 py-2.5 bg-primary-500 text-white rounded-lg text-sm font-medium disabled:opacity-50">
...
function I({label,value,onChange,ph,type='text',disabled=false,r=false}:{label:string;value:string;onChange:(v:string)=>void;ph?:string;type?:string;disabled?:boolean;r?:boolean}) {
  return <div><L r={r}>{label}</L>
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={ph} disabled={disabled}
      className={`w-full px-3 py-2.5 rounded-lg border text-sm outline-none disabled:bg-gray-50 ${r&&!value.trim()?'border-red-300':'border-gray-200 focus:border-primary-500'}`}/></div>
}
  ...
  return <div><L>{label}</L>
    <div className="flex gap-2 mt-1">{options.map(o=>
      <button key={o} type="button" onClick={()=>onChange(o)}
        className={`flex-1 py-2.5 rounded-lg text-sm border text-center ${value===o?'bg-primary-500 text-white border-primary-500':'bg-white text-gray-600 border-gray-200'}`}>{o}</button>
    )}</div></div>
}
function FileUp({label,onChange,required=false}:{label:string;onChange:(f:File|null)=>void;required?:boolean}) {
  ...
  return <div>
    ...
    <div className="flex gap-2">
      <label className="flex items-center gap-2 px-3 py-2.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 flex-1 min-w-0">
        <span className="text-gray-400 text-lg flex-shrink-0">📎</span>
        <span className="text-sm text-gray-500 truncate">{fname||'添加附件'}</span>
        <input type="file" className="hidden" onChange={e=>{const f=e.target.files?.[0]||null;onChange(f);setFname(f?f.name:'')}}/>
      </label>
      <button type="button" className="px-3 py-2.5 border border-gray-200 rounded-lg hover:bg-gray-50 flex-shrink-0">
...
function IdTab({form,up,files,handleFile,toPinyin,splitAddress,translateAddress,openDatePicker}:{
  ...
  return <div className="space-y-4">
    ...
    {form.namePinyin && <p className="text-xs text-primary-500">拼音：{form.namePinyin}</p>}
  ...
  return <div className="space-y-4">
    ...
    <div><L r>申请ITIN的原因/用途</L>
      <div className="flex flex-wrap gap-2 mt-1">{purposeOpts.map(p=>
        <label key={p} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm border cursor-pointer ${form.itinPurpose.includes(p)?'bg-primary-50 border-primary-300 text-primary-700':'bg-white border-gray-200 text-gray-600'}`}>
